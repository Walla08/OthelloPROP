...
public class PlayerID implements IPlayer, IAuto {
    ...
    public void timeout() {
        timeout = true;
        System.out.println("TIMEOUT BITCH");
    }
            ...
            while(!timeout){
                ...
                for(int movi = 0; movi < moves.size(); ++movi){
                    ...
                    if(x.canMove(moves.get(movi), s.getCurrentPlayer())){
                        ...
                        ++this.profundidad;
                        if(timeout){
                            this.millormov = moves.get(movi);
                            break;
                        }
                        if (actual > max && !timeout) {
                            max = actual;
                            this.movio = moves.get(movi);

                        }
                    }
                }
            }
            System.out.println("Punto anterior "+this.millormov);
            System.out.println("Punto actual "+this.movio);
            this.movio = this.millormov;
        }
        return new Move( movio,0 , this.profundidad, SearchType.RANDOM);
    }
    ...
    public String getName() {
     ...
     public int minimax(GameStatus s, int profunditat, int alpha, int beta, boolean esBendicion) {
        ...
        for (int movi = 0; movi < moves.size() && !timeout; movi++) {
            // Copia del tablero
            GameStatus x = new GameStatus(s);
    ...
    public int heuristica(GameStatus s, ArrayList<Point> moves) {
        ...
        System.out.println("Player "+s.getCurrentPlayer());
        heuris = max;
        return heuris;
    }
